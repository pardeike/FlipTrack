    ...
    static func read(_ input: CIImage) throws -> [DisplayText] {
        ...
        for rectangle in rectangles.results ?? [] {
            ...
            if EndGameLayout.result(in: text) != nil || GameDisplayLayout.isNewGame(in: text) ||
                GameDisplayLayout.isBonusScreen(in: text) { matches.append(text) }
        ...
        if EndGameLayout.result(in: fullText) != nil || GameDisplayLayout.isNewGame(in: fullText) || GameDisplayLayout.isBonusScreen(in: fullText) { return fullText }
