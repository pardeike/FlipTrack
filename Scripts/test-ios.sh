...
xcodebuild -project .build/FlipTrackDeviceTests.xcodeproj -scheme FlipTrackDeviceHost -destination "id=$DEVICE" -derivedDataPath .build/device-tests -resultBundlePath "$RESULT" -allowProvisioningUpdates test >> "$LOG" 2>&1
