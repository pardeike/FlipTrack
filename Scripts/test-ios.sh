...
TEST_FILTER="${2:-FlipTrackDeviceUITests}"
mkdir -p .build/logs
...
xcodebuild -project .build/FlipTrackDeviceTests.xcodeproj -scheme FlipTrackDeviceHost -destination "id=$DEVICE" -derivedDataPath .build/device-tests -resultBundlePath "$RESULT" -allowProvisioningUpdates test "-only-testing:$TEST_FILTER" >> "$LOG" 2>&1
